...
static inline unsigned char even_parity(unsigned int data)
{
...
static unsigned int pocsag_syndrome(unsigned int data)
{
    unsigned int shreg = data >> 1; /* throw away parity bit */
    unsigned int mask = 1L << (BCH_N-1), coeff = BCH_POLY << (BCH_K-1);
    int n = BCH_K;
...
{
    // LOGD("Im in ...pocsag_printmessage....Ok...confused..!");
    ...
    if((s->l2.pocsag.address != -1) || (s->l2.pocsag.function != -1))
    {
        ...
        else
        {
            ...
            if((pocsag_mode == POCSAG_MODE_SKYPER) || ((pocsag_mode == POCSAG_MODE_AUTO) && (guess_skyper >= 20 || unsure)))
            {
                ...
                verbprintf(0, "%s: Address: %7lu  Function: %1hhi  ",s->dem_par->name, s->l2.pocsag.address, s->l2.pocsag.function);
                }
                else
                    verbprintf(0, "%s: Address:       -  Function: -  ",s->dem_par->name);
                if(pocsag_mode == POCSAG_MODE_AUTO)
...
{
    // LOGD("Im in ...pocsag_deinit....This is never happening..!");
...
static unsigned int *transpose_clone(unsigned int src, unsigned int *out)
{
    int i;
    if (!out)
        out = malloc(sizeof(unsigned int)*32);
...
static void bitslice_syndrome(unsigned int *slices)
{
    ...
    int i, n;
    unsigned int paritymask = slices[0];
...
int pocsag_brute_repair(struct l2_state_pocsag *rx, unsigned int* data)
{

    if (pocsag_syndrome(*data)) {
        rx->pocsag_total_error_count++;
        verbprintf(6, "Error in syndrome detected!\n");
       // LOGD("Error in syndrome detected!");
    ...
    // check for single bit errors
    {
        int i, n, b1, b2;
        unsigned int res;
        unsigned int *xpose = 0, *in = 0;

        xpose = malloc(sizeof(unsigned int)*32);
        in = malloc(sizeof(unsigned int)*32);
        ...
        for (b1 = 0; b1 < 32; ++b1) {
            for (b2 = b1; b2 < 32; ++b2) {
                ...
                if (++n == 32) {
                    memcpy(in, xpose, sizeof(unsigned int)*32);
...
{
    // LOGD("Im in ...word_complete....When the word is complete let the program counter pass..!");
...
static inline bool is_sync(const unsigned int * const rx_data)
{
    // LOGD("Im in ...is_sync....Good..!");
    if(*rx_data == POCSAG_SYNC)
        return true; // Sync found!
    return false;
}

static inline bool is_idle(const unsigned int * const rx_data)
{
    // LOGD("Im in ...is_idle....NO..!");
