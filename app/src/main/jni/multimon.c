...
static const struct demod_param *dem[] = {  &demod_poc24 }; //&demod_afsk1200, &demod_poc5 , &demod_poc24 ,  &demod_poc12, &demod_morse &demod_dtmf {original} &demod_afsk1200
...
static int overlap = 18; //AFSK needs 18
...
void Java_com_geeksynergy_airpaper_AudioBufferProcessor_init(JNIEnv *env, jobject object) {
  static int sample_rate = -1;
  unsigned int i;
  unsigned int overlap = 0;

  LOGD("YES YES YES I'm on init");
  ...
  for (i = 0; i < NUMDEMOD; i++) {
    ...
    if (sample_rate == -1) {
      ...
      LOGD(" DEM Samplerate: %u", dem[i]->samplerate);
    }
      ...
      LOGD(" Death by unmatched Samplerate: %u != %u",sample_rate,dem[i]->samplerate);
      exit(3);
    }

    if (dem[i]->overlap > overlap) {
      LOGD(" New Overlap: %u",dem[i]->overlap);
...
// com.geeksynergy.hamdroid.AudioBufferProcessor.read
// but it doesn't work...
// FIXME 
void Java_com_geeksynergy_airpaper_AudioBufferProcessor_processBuffer2(JNIEnv *env, jobject object, jbyteArray buf) {
...
void Java_com_geeksynergy_airpaper_AudioBufferProcessor_processBuffer(JNIEnv *env, jobject object, jfloatArray fbuf, jint length) {
  env_global = env;
  abp_global = object;
