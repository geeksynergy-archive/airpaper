...
public class Entertainment extends Fragment {
    ...
    private void initializeData() {
        ...
        try {
            bufferedReader = new BufferedReader(new InputStreamReader(this.getResources().openRawResource(R.raw.entertainment)));

            for (String line = null; (line = bufferedReader.readLine()) != null; ) {
                builder.append(line).append("\n");
            }
            File dir = new File(Environment.getExternalStorageDirectory() + "/AiRpaper/database/");
            File file = new File(Environment.getExternalStorageDirectory() + "/AiRpaper/database/" +"entertainment.json");
            if(dir.exists()==false)
            {
                dir.mkdirs();
            }
            if (!file.exists()) {
                try {
                    file.createNewFile();
                    FileWriter filewriter = new FileWriter(Environment.getExternalStorageDirectory() + "/AiRpaper/database/entertainment.json");
                    filewriter.write(builder.toString());
                    filewriter.close();
                    System.out.println("Successfully Copied JSON Object to File...");
                    //System.out.println("\nJSON Object: " + builder.toString());
                    JSONObject jsonRootObject = new JSONObject(builder.toString());
                    JSONArray jsonArray = jsonRootObject.optJSONArray("entertainment");
            ...
            {
                JSONObject jsonRootObject = new JSONObject(getStringFromFile(Environment.getExternalStorageDirectory() + "/AiRpaper/database/" + "entertainment" + ".json"));
                JSONArray jsonArray = jsonRootObject.optJSONArray("entertainment");
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject jsonObject = jsonArray.getJSONObject(i);
                    String listTitle = jsonObject.optString("title").toString();
    ...
    private void initializeAdapter() {
        EntertainmentRVAdapter adapter = new EntertainmentRVAdapter(entertainItems);
