...
public class Business extends Fragment {
    ...
    private void initializeData() {
        ...
        try {
            ...
            File file = new File(Environment.getExternalStorageDirectory() + "/AiRpaper/database/" +"business.json");
            if(dir.exists()==false)
            {
                dir.mkdirs();
            }
            if (!file.exists()) {
                try {
                    file.createNewFile();
                    FileWriter filewriter = new FileWriter(Environment.getExternalStorageDirectory() + "/AiRpaper/database/business.json");
            ...
            {
                JSONObject jsonRootObject = new JSONObject(getStringFromFile(Environment.getExternalStorageDirectory() + "/AiRpaper/database/" + "business" + ".json"));
                JSONArray jsonArray = jsonRootObject.optJSONArray("business");
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject jsonObject = jsonArray.getJSONObject(i);
                    String listTitle = jsonObject.optString("title").toString();
